...
import { fetch_llms_txt } from "../../src/tools/fetch_llms_txt.js";
...
vi.mock("../../src/lib/utils.js", async () => {
  ...
  return {
    ...actual, // Keep all original exports including getVersion
...
describe("fetch_llms_txt", () => {
  ...
  beforeEach(() => {
    ...
    process.env.MCP_STDIO_MODE = "silent"; // Default to silent to avoid console noise
  });
  ...
  it("should handle a single URL in object format", async () => {
    ...
    vi.mocked(utils.fetchContent).mockResolvedValueOnce("Content from llms.txt");
    
    // Call function with object input
    ...
    expect(utils.checkDomainAccess).toHaveBeenCalledWith(mockTargetInfo, mockAllowedDomains);
    expect(utils.fetchContent).toHaveBeenCalledWith(mockTargetInfo);
    
    expect(result).toEqual({
      content: [
        { type: "text", text: "Content from llms.txt" }
      ]
    });
  });
  ...
  it("should log to console when MCP_STDIO_MODE is not silent", async () => {
    ...
    await fetch_llms_txt({ url: "https://example.com/llms.txt" }, mockExtra, mockAllowedDomains);
    ...
    expect(mockConsoleInfo).toHaveBeenCalledWith(
      expect.stringContaining("Fetching llms.txt from https://example.com/llms.txt")
    );
  });
  ...
  it("should handle domain access check failures", async () => {
    ...
    vi.mocked(utils.checkDomainAccess).mockImplementationOnce(() => {
      throw new Error("Access denied: Domain 'untrusted.example' is not allowed");
    });
  ...
  it("should handle content fetch failures", async () => {
    ...
    vi.mocked(utils.parseFetchTarget).mockResolvedValueOnce(mockTargetInfo);
    vi.mocked(utils.fetchContent).mockRejectedValueOnce(new Error("HTTP error 404"));
