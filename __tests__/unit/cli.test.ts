...
import { parseCliArgs, type CliConfig } from "../../src/lib/cli.js";
...
describe("CLI Module", () => {
  ...
  beforeEach(() => {
    ...
    vi.mocked(cliLib.addParsedSourceToTarget).mockImplementation((parsed, target, flag) => {
      target[parsed.name] = parsed.urlValue;
    });
    
    vi.mocked(cliLib.normalizeAndAddDomain).mockImplementation((domain, set, flag) => {
      set.add(domain);
    });
