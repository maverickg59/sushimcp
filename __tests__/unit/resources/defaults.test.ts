...
describe("Defaults Resources Module", () => {
  ...
  it("should process defaults resources correctly", () => {
    ...
    const typescriptResource = resources.find(
      (r) => r.id === "typescript_docs",
    );
    expect(typescriptResource).toBeDefined();
    expect(typescriptResource?.title).toBe("Typescript  Docs");
    expect(typescriptResource?.description).toBe("Typescript docs llms.txt");
    expect(typescriptResource?.mimeType).toBe("text/plain");
    expect(typescriptResource?.uri).toBe(
      "https://example.com/typescript/llms.txt",
    );
  ...
  it("should transform resource name correctly", () => {
    ...
    expect(reactResource?.title).toBe("React  Docs");

    const nodejsResource = resources.find((r) => r.id === "nodejs_docs");
    expect(nodejsResource?.title).toBe("Nodejs  Docs");
  });
  ...
  it("should handle resource handler calls correctly", async () => {
    ...
    if (result) {
      ...
      expect(result.contents[0].text).toBe(
        "Mock content for https://example.com/typescript/llms.txt",
      );
    }
  });
  ...
  it("should handle fetch errors in handler", async () => {
    ...
    await expect(typescriptResource?.handler(mockUrl)).rejects.toThrow(
      "Network error",
    );
  });
  ...
  it("should handle complex resource names with multiple words", () => {
    ...
    vi.mocked(fs.readFileSync).mockReturnValue(
      JSON.stringify({
        complex_resource_name_with_multiple_words:
          "https://example.com/complex/llms.txt",
    ...
    const complexResource = resources.find(
      (r) => r.id === "complex_resource_name_with_multiple_words",
    );
    expect(complexResource?.title).toBe(
      "Complex Resource Name With Multiple Words",
    );
    expect(complexResource?.description).toBe(
      "Complex resource name with multiple words llms.txt",
    );

    const hyphenatedResource = resources.find(
      (r) => r.id === "hyphenated_resource_name",
    );
    expect(hyphenatedResource?.title).toBe("Hyphenated Resource Name");
    expect(hyphenatedResource?.description).toBe(
      "Hyphenated resource name llms.txt",
    );
  });
});
