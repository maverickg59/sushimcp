...
function parseUrlOption(
  optionString: string
): { name: string; urlValue: string } | null {
...
function addSource(
  ...
  try {
    ...
    if (urlValue.startsWith("/") || urlValue.startsWith(".")) {
      ...
      console.error(
        `(${sourceOrigin}) Invalid URL or Path format for source '${name}': '${urlValue}'. Skipping. Error: ${
  ...
  if (allowDomainOptions && allowDomainOptions.length > 0) {
    ...
    Object.values(docSources).forEach((url) => {
      try {
        ...
        console.error(
          `Warning: Could not parse source URL '${url}' for default domain inference. Error: ${
    ...
    if (allowedDomains.size === 0 && Object.keys(docSources).length > 0) {
      console.error(
        "Warning: No remote URLs configured or parsed, and no explicit --allow-domain. Fetching might be restricted to local files only."
      );
...
export function parseCliArgs(): CliConfig {
  ...
  const docSources = processUrlOptions(
    options.url,
    options.urls,
    includeDefaults
  );
  const allowedDomains = processAllowDomainOptions(
    options.allowDomain,
    docSources
  );
  ...
  if (process.env.MCP_STDIO_MODE) {
    ...
    console.info(
      `Final Allowed Fetch Domains: ${
        [...allowedDomains].join(", ") || "(None - local only?)"
      }`
    );
    console.info("------------------------------------\\n");
  }
